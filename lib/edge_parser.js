			...
			registerCompositionDefn: function (compId, symbols, fonts, scripts, resources, opts){
				...
				function recurseObj(obj){
					for (var i=0; i<obj.length; i++){
						...
						} else if (obj[i].fill && obj[i].fill.length > 1 && typeof obj[i].fill[1] == 'object'){//we need the gradient js for it to actually add the gradient!
							if (_this.propertiesAnimated.indexOf('background-image') == -1){
								_this.propertiesAnimated.push('background-image');
							}
						}
